...
- S3 bucket `tacedata-s3-bucket-02/tacedata-site/` configured with CloudFront OAC bucket policy
- CloudFront distribution `E1144JDZXTVLFW` — `d2i8qth3t7mbtl.cloudfront.net`
- IAM OIDC provider for GitHub Actions (`token.actions.githubusercontent.com`)
- IAM role `tacedata-github-deploy` — least-privilege, scoped to this repo and distribution
