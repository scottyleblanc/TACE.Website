...
Before touching a nameserver, I needed to inventory everything that depended on the current DNS setup — not just the website, but email too. tacedata.ca has Fastmail MX records, DKIM CNAME records, SPF and DMARC TXT records. All of it had to be replicated in Route 53 before the nameservers changed. If I missed one, email would break the moment the delegation propagated.
...
One thing worth understanding: domain registration and DNS hosting are separate concerns. I kept Websavers as the registrar — they hold the domain registration — but delegated DNS authority to Route 53 by changing the nameservers. Route 53 is now authoritative for all DNS on tacedata.ca. Websavers is just where the domain is registered.

This is the right way to do it. Transferring the registration itself is a separate process (5-7 days, ICANN window) and is not required for any of this to work.

## ACM and CloudFront

AWS Certificate Manager issues free SSL certificates, but there is a catch: CloudFront only accepts certificates provisioned in `us-east-1`, regardless of where your other AWS resources live. The certificate has to be requested in that region.
...
While the certificate was pending, I needed to check its status periodically until it reached ISSUED. The manual version is running the same AWS CLI command every few minutes and reading the output. Instead, I wrote a small PowerShell loop:
...
Run it, walk away, come back when it stops. The pattern — `do { check; if done break; sleep } while ($true)` — is reusable anywhere you are waiting on an external process to reach a target state. I will use this again.

## the runbook

I wrote a full runbook for this stage with every command, all resource IDs, and a step-by-step procedure. If you are doing something similar — Hugo or otherwise — it covers the end-to-end DNS cutover for a static site on S3 + CloudFront.
